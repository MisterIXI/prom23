...
def detect(save_img=False):
    ...
    ## Start of custom changes
                # ## publish results to pub
                # try:
                #     print(f"Starting building image message")
                #     image_message = bridge.cv2_to_imgmsg(im0, encoding="bgr8")
            ...
            msg.height = im0.shape[0]
            msg.width = im0.shape[1]
            msg.encoding = "bgr8"
            msg.is_bigendian = 0
            # msg.step = 3 * msg.width
            # msg.data = np.array(im0).tobytes()
            msg.data = im0.tostring()
            msg.step = len(msg.data) // msg.height
            global pub
            # while True:
            pub.publish(msg)
            print(f"msg.data dimensions: {len(msg.data)}")
                # cv2.imwrite("classified_horses.jpg",im0) 
    #            # Stream results
                # if view_img:
...
## start custom changes
    ...
    # bridge = CvBridge()
    # set up ros node
    print(f"Starting node and publishing topic")
    rospy.init_node('yolo', anonymous=True)
    # set up publisher
    global pub
    pub = rospy.Publisher('/yolo', Image, queue_size=10)
    global cam
    cam = CSICamera(width=1280, height=736, capture_width=1280, capture_height=720, capture_fps=30)
    # cam.set_gain(10)
    # cam.set_exposure_time(30000)
## end custom changes
    with torch.no_grad():
